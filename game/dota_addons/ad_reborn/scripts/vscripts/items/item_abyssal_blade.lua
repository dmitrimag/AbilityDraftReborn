...
function modifier_item_abyssal_blade_datadriven_bash_chance_on_attack_landed(keys)
	if not keys.caster:HasModifier("bash_cooldown_modifier") then
		...
		if ( not HasAbilities( keys.caster, GameMode.AbilitiesBashKV ) ) and ((is_ranged_attacker and random_int <= keys.BashChanceRanged) or (not is_ranged_attacker and random_int <= keys.BashChanceMelee)) then
